module Ri.Parsing.Attoparsec.ByteString
(
  uri,
  httpUri,
  regName,
...
import Ri.Prelude hiding (foldl, hash)
import Ri.Data
...
import qualified Ri.PercentEncoding as I
import qualified Ri.CodePointPredicates.Rfc3986 as C
import qualified Ri.MonadPlus as R
...
uri =
  labeled "URI" $ do
    ...
    return (Uri parsedScheme parsedHierarchy parsedQuery parsedFragment)

{-|
Same as 'uri', but optimized specifially for the case of HTTP URIs.
-}
{-# INLINABLE httpUri #-}
httpUri :: Parser HttpUri
httpUri =
  labeled "HTTP URI" $ do
    ...
    parsedQuery <- query
    parsedFragment <- fragment
    return (HttpUri (Security secure) parsedHost parsedPort parsedPath parsedQuery parsedFragment)
